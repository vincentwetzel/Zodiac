...
class ZodiacApp:
    def __init__(self):
        ...
        try:
            ...
            save_file = tkinter.filedialog.asksasveasfilename(title="Choose a name for your ZodiacInfo database",
        ...
        while True:
            ...
            if user_input == 1:
                ...
                # 4. Look up a sign's information"
                print(ZodiacInfo.get_sign_info(ZodiacInfo.ZodiacSigns[input(
                    "The signs of the ZodiacInfo are Capricorn, Aquarius, Pisces, Aries, Taurus, Gemini, "
                    "Cancer, Leo, Virgo, Libra, Scorpio, Sagittarius."
                    "\nEnter the sign that you would like to know more about: ")]))
            elif user_input == 5:
                # 5. Look up a symbol
                print(format_section(ZodiacInfo.get_symbol_info(input(
                    "Enter a ZodiacInfo sign and I will give you its symbol."
                    "\nYour options are:"
                    "\nCapricorn, Aquarius, Pisces, Aries, Taurus, Gemini, "
                    "Cancer, Leo, Virgo, Libra, Scorpio, Sagittarius: "))), "*")
            elif user_input == 6:
                # 6. Look up an element.
                print(format_section(ZodiacInfo.get_element_info(input(
                    "Enter a ZodiacInfo element and I will give you information about it."
                    "\nYour options are:"
                    "\nAir, Fire, Earth, and Water: "))), "*")
            elif user_input == 7:
                # 7. Look up a date.
                print(format_section(ZodiacInfo.get_sign_for_date_str(
                    input("Enter a day of the year and I will give you its ZodiacInfo information (e.g. January 1): "))),
                    "*")
            elif user_input == 8:
                # 8. Play a guessing game for signs and dates.
                pass
    ...
    def init_people_database(self, excel_file) -> None:
        """
        Initializes ZodiacInfo data from a CSV file.
        :param excel_file:
        :return: None
        """
        df = pandas.read_excel(excel_file)
        for idx, row in df.iterrows():
            person:ZodiacPerson = ZodiacPerson(row["Name"],
                                  datetime.date(int(row["Year"]),
                                                ZodiacInfo.month_str_to_int(row["Month"]),
                                                int(row["Day"])),
    ...
    def get_stats_about_database(self):
        """
        Gets statistics about the ZodiacInfo database.
        ...
        output += self.get_str_for_dict_of_lists(self.people_by_sign_dict)
    ...
    def get_person_info_from_db(self, person_name):
        ...
        :return: A str containing ZodiacInfo information about that person.
        """
        output = ""
        person_found = False
    ...
    def get_str_for_dict_of_lists(self, d):
        ...
        """
        Prints info about a dictionary of ZodiacInfo people.
